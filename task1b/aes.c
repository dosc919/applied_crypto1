...
void aes_encrypt(const BYTE in[], BYTE out[], const WORD key[], int num_rounds)
{
	BYTE state[4][4];
	...
	{
		SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[i * 4]);
	}
	SubBytes(state); ShiftRows(state); AddRoundKey(state,&key[i * 4]);
